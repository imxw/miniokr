...
gen.ca: ## 生成 CA 文件.
	...
	@openssl req -new -key $(OUTPUT_DIR)/cert/ca.key -out $(OUTPUT_DIR)/cert/ca.csr \
		-subj "/C=CN/ST=Beijing/L=Beijing/O=devops/OU=it/CN=127.0.0.1/emailAddress=ixw1991@126.com" # 2. 生成请求文件
	...
	@openssl req -new -key $(OUTPUT_DIR)/cert/server.key -out $(OUTPUT_DIR)/cert/server.csr \
		-subj "/C=CN/ST=Beijing/L=Beijing/O=serverdevops/OU=serverit/CN=127.0.0.1/emailAddress=ixw1991@126.com" # 6. 生成服务端向 CA 申请签名的 CSR
