...
使用方式 (对比2次测试结果)
1. 性能测试：./wrktest.sh http://127.0.0.1:8999/healthz
2. 执行命令： ./wrktest.sh diff miniokr.dat http.dat
...
Performance automation test script.
  ...
  URL                    HTTP request url, like: http://127.0.0.1:8999/healthz
  diff                   Compare two performance test results

OPTIONS:
  -h                     Usage information
  -n                     Performance test task name, default: miniokr
  -d                     Directory used to store performance data and gnuplot graphic, default: _output/wrk

Reprot bugs to <ixw1991@126.com>.
EOF
}
